...
// Invoke npm through its JavaScript CLI. This avoids Windows .cmd resolution
// differences between PowerShell and Node child processes while preserving .npmrc auth.
const npmCliPath =
...
if (!fs.existsSync(npmCliPath)) {
  throw new Error(`Could not locate npm's JavaScript CLI at ${npmCliPath}.`);
}
...
function npmOutput(args) {
  return output(process.execPath, [npmCliPath, ...args]);
}
...
if (isDryRun) {
  console.log(`\nDry-run summary:`);
  console.log(
    `  GitHub branch: ${existingTagMatchesHead ? "would push main only" : `would create and push ${tagName}`}`,
  );
  console.log(
    `  npm package: ${versionAlreadyPublished ? `${packageName}@${version} already exists; a real deploy would stop` : `would publish ${packageName}@${version}`}`,
  );
  process.exit(0);
}
...
runNpm(publishArgs());
const publishedVersion = npmOutput(["view", `${packageName}@${version}`, "version"]);
