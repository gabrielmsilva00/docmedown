...
export const Navbar: React.FC = () => {
  const { config, setIsSearchOpen, isMobileSidebarOpen, setIsMobileSidebarOpen, home } = useDoc();
  const { resolvedMode, toggleMode } = useTheme();

  const homeHref = home ? (home.kind === "href" ? (home.href ?? "#/") : `#/${home.slug}`) : "#/README";
  const homeTitle = home?.via === "ancestor" ? "Original documentation home" : "Documentation home";
  ...
  return (
    <header className="dmd-navbar">
      <div className="dmd-navbar-left">
        ...
        <a href={homeHref} className="dmd-brand">
      ...
      <div className="dmd-navbar-right">
        {/* Home */}
        {home && (
          <a href={homeHref} className="dmd-social-btn dmd-home-btn" title={homeTitle}>
            <svg
              width="18"
              height="18"
              viewBox="0 0 24 24"
            ...
            <span className="dmd-sr-only">{homeTitle}</span>
          </a>
        )}
