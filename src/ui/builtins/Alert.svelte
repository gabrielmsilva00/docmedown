<svelte:options customElement={{ tag: "dmd-alert" }} />
<script lang="ts">
  ...
  let { type = "note", title = "" }: { type?: string; title?: string } = $props();
  ...
  const icon = $derived(ALERT_ICONS[type] ?? ALERT_ICONS.note);
</script>

<div bind:this={root} class="dmd-alert dmd-alert-{type}" role="note">
  <div class="dmd-alert-head">
    <span class="dmd-alert-icon" aria-hidden="true">{@html icon}</span>
    {#if title}
      <span class="dmd-alert-title">{title}</span>
    {/if}
  </div>
