...
export class DmdAlert extends LitElement {
  static properties = {
    type: { type: String },
    title: { type: String },
  };
  ...
  render() {
    const icon = ALERT_ICONS[this.type] ?? ALERT_ICONS.note;
    return html`
      <div class="dmd-alert dmd-alert-${this.type}" role="note">
        <div class="dmd-alert-head">
          <span class="dmd-alert-icon" aria-hidden="true">${unsafeHTML(icon)}</span>
          ${this.title ? html`<span class="dmd-alert-title">${this.title}</span>` : ""}
        </div>
        <div class="dmd-alert-body"><slot></slot></div>
      </div>
    `;
  }
}

export class DmdCallout extends DmdAlert {}
customElements.define("dmd-alert", DmdAlert);
customElements.define("dmd-callout", DmdCallout);
