...
import { scanDirectory, generateManifest, extractHeadings } from '../src/cli/utils/scanner';
...
test('Nested documentation roots are isolated from the parent manifest', () => {
  const docsDir = path.resolve(__dirname, '../docs');
  const files = scanDirectory(docsDir);
  const nestedRoots = findNestedDocumentationRoots(docsDir).map((root) => path.relative(docsDir, root).replace(/\\/g, '/'));
